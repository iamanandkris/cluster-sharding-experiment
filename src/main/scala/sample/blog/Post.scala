...
import message.{Create, UpdateName, UpdateOwner}
    ...
    case AddPost(_, content) =>
      if (content.author != "" && content.title != "")
        persist(PostAdded(content)) { evt =>
          ...
          accountRegion ! Create(Some(Account(s"name-${accountId}",s"chs-${accountId}","ROOT",List("Anand","Nathan","Marek"))),accountId)
  ...
  def created: Receive = {
    ...
    case ChangeBody(_, body) =>
      persist(BodyChanged(body)) { evt =>
        state = state.updated(evt)
        accountRegion ! UpdateName(s"NewName-${accountId}",accountId)
    ...
    case Publish(postId) =>
      persist(PostPublished) { evt =>
        ...
        context.become(published)
        val c = state.content
        accountRegion ! UpdateOwner(s"NewOwner-${accountId}",accountId)
